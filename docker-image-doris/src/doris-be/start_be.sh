...
if [ -n "$AUTO_REGISTER" ]; then
  RUN_AUTO_REGISTER=${AUTO_REGISTER}
fi
echo "RUN_AUTO_REGISTER=${RUN_AUTO_REGISTER}"
...
if [[ "${RUN_AUTO_REGISTER}" == "1" ]]; then
    echo "======开始注册 be!====="
    mysql -h${FE_MASTER_IP} -P${FE_MASTER_PORT} -uroot -p${FE_ROOT_PWD} -e "ALTER SYSTEM ADD BACKEND '${BE_IPADDRESS}:${BE_PORT}'"
    echo "=====检查 be 状态!====="
    mysql -h${FE_MASTER_IP} -P${FE_MASTER_PORT} -uroot -p${FE_ROOT_PWD} -e "SHOW PROC '/backends';"
fi
